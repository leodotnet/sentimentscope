...
java -Xmx32g -jar sentimentspan.jar baseline_pipelineNE 1000 $l2 $beginIndex $endIndex $lang weightnotpush $task $NElen > $logfile 2>&1

python scripts/sentiment_pipeline.py $beginIndex $endIndex $lang $task

java -Xmx32g -jar sentimentspan.jar baseline_pipelineSent 1000 $l2 $beginIndex $endIndex $lang weightnotpush $task $NElen > $logfile 2>&1

python scripts/sentiment.py $beginIndex $endIndex baseline_pipeline $lang $task N
