...
import com.nlp.targetedsentiment.f.pipelineSent.PipeTargetSentimentCompiler.*;
...
public class PipeTargetSentimentFeatureManager extends LinearFeatureManager {
	...
	public enum WORD_FEATURE_TYPES {
		_id_, ne, brown_clusters5, jerboa, brown_clusters3, _sent_, _sent_ther_, sentiment, postag, NE_Sent, NE_predicted, word_index
	};
		...
		if (ids_parent[4] == NodeType.Span.ordinal()) //!input_token[pos_parent].getName().startsWith("O")
		{
			...
			if (featureArr.size() > 0) {
				...
				for (String[] item : featureArr) {
					...
					if  (!item[0].contains("_sent_"))
						feature.add(new String[]{item[0] + item[2], Out,  "_link(NE)"});
