package com.nlp.targetedsentiment.f.pipelineSent;
...
public class PipeTargetSentimentCompiler extends LinearNetworkCompiler {
	...
	public enum SubNodeType {O, positive, negative, neutral}
	...
	public LinearInstance decompile(Network network) {
		...
		for(int i = 0; i < preds.size(); i++)
		{
			...
			String output = "O";
			if (!input[pos].getName().startsWith("O"))
				output = input[pos].getName() + "-" + subnode;
			
			predication_array.add(new OutputTag(output));	
	...
	private LinearNetwork compileUnlabeled(int networkId, LinearInstance inst, LocalNetworkParam param){
		...
		char currentNE = 0;
		char nextNE = inputs[0].getName().charAt(0);
		//nextNE = 'B';
		...
		{
			for(int j = 1; j < SubNodeTypeSize; j++)
			{
				from = start;
				to = node_array[0][j];
				network.addEdge(from, new long[]{to});
		...
		for(int pos = 0; pos < size - 1 ; pos++)
		{
			nextNE = inputs[pos + 1].getName().charAt(0);
			//nextNE = 'B';
		...
		if (visual)
			visualize(network, "Pipeline Model:unlabeled", networkId);
	...
	private LinearNetwork compileLabeled(int networkId, LinearInstance inst, LocalNetworkParam param){
		...
		for(int pos = 0; pos < size; pos++)
		{
			...
			subnode = SubNodeType.valueOf(sent);
		...
		if (visual)
			visualize(network, "Pipeline Model:labeled", networkId);
