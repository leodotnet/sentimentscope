...
//		System.exit(1);
	...
	private LinearNetwork compileLabeled(int networkId, LinearInstance inst, LocalNetworkParam param){
				...
				if (last_entity_pos == -1)
				{
					...
					for(int i = last_entity_pos + 1; i < pos; i++)
					{
						if (i <= mid)
						{
							...
							from = node_array[i - 1][last_polar.ordinal()][SubNodeType.A.ordinal()];
							to = node_array[i][polar.ordinal()][SubNodeType.B.ordinal()];
							network.addEdge(from,  new long[]{to});
							...
							from = node_array[i - 1][polar.ordinal()][SubNodeType.B.ordinal()];
							to = node_array[i][polar.ordinal()][SubNodeType.B.ordinal()];
							network.addEdge(from,  new long[]{to});
						}
					}
					...
					{
						from = node_array[last_entity_pos][last_polar.ordinal()][SubNodeType.A.ordinal()];
						to = node_array[pos][polar.ordinal()][SubNodeType.B.ordinal()];
						network.addEdge(from,  new long[]{to});
					} else {
						from = to;
						to = node_array[pos][polar.ordinal()][SubNodeType.B.ordinal()];
		...
		if (polar != null)
		{
			...
			System.err.println("No Entity found or too long entity (len:" + len + ") in this Instance, Discard!");
			
			for(int pos = 0; pos < size; pos++)
			{
				System.err.print(inputs[pos].getName() + " ");
			}
			System.err.println();
