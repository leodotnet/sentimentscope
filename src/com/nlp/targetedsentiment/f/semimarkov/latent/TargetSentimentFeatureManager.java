...
import com.nlp.targetedsentiment.f.semimarkov.latent.TargetSentimentCompiler.*;
...
public class TargetSentimentFeatureManager extends LinearFeatureManager {
	...
	public FeatureArray extract_helper(Network network, int parent_k, int[] children_k) {
		...
		ArrayList<String> WordEmbeddingFeatures = new ArrayList<String>();
		...
		if (nodetype_parent == NodeType.Span.ordinal() && nodetype_child == NodeType.Span.ordinal())
		{
			...
			for(int k = segment_start; k <= segment_end; k++)
			{
				...
				WordEmbeddingFeatures.add(word.toLowerCase());
		...
		if (feature.size() > 0)
		{
			if (TargetSentimentGlobal.ENABLE_WORD_EMBEDDING)
			{
				...
				int f[] = new int[normal_feature_size + (WordEmbeddingFeatures.isEmpty() ? 0 : TargetSentimentGlobal.Word2Vec.ShapeSize)];
				...
				if (!WordEmbeddingFeatures.isEmpty())
				{
					for(String word : WordEmbeddingFeatures)
					{
						Double[] vector = TargetSentimentGlobal.Word2Vec.getVector(word);
						if (vector == null)
						{
