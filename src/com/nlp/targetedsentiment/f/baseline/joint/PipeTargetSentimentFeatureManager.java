...
import com.nlp.targetedsentiment.f.baseline.joint.PipeTargetSentimentCompiler.*;
...
public class PipeTargetSentimentFeatureManager extends LinearFeatureManager {
	...
	public FeatureArray extract_helper(Network network, int parent_k,
		...
		if (ids_parent[4] == NodeType.Span.ordinal()) //!input_token[pos_parent].getName().startsWith("O")
		{
			...
			if (featureArr.size() > 0) {
				...
				for (String[] item : featureArr) {
					...
					feature.add(new String[]{item[0] + item[2], Out,  "_link(NE,SENT)"});
		...
		if (feature.size() > 0) {
			...
			int inst_id = network.getInstance().getInstanceId() - 1;
			//System.out.println("pos:" + pos + " size:" + size + " inst_features[inst_id] size:" + PipeTargetSentimentAlgoModel.inst_features[inst_id].length);
			//System.out.println("");
			PipeTargetSentimentAlgoModel.inst_features[inst_id][pos] = f.clone();
		}
		return fa;
	}
